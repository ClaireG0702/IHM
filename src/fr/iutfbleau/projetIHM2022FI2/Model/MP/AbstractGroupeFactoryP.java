...
public class AbstractGroupeFactoryP implements AbstractGroupeFactory {
     ...
     * fermer la conn avec fin
     */
    public AbstractGroupeFactoryP(String name, int min, int max){
        Objects.requireNonNull(name,"On ne peut pas créer une promotion  dont le nom est null");
        try{	//connection
			Class.forName("org.mariadb.jdbc.Driver");
			}catch(ClassNotFoundException e){
			System.err.println("forname marche pas");
			}

		try{
			this.conn = DriverManager.getConnection(
				"jdbc:mariadb://dwarves.iut-fbleau.fr/dossantoad","dossantoad","314671");
			}catch(SQLException e1){
        	System.err.println("conn marche pas");
        }

        this.promo=new GroupeP(1,this.conn);
    ...
    public void createGroupe(Groupe pere, String name, int min, int max){
        ...
        Groupe g = new GroupeP(pere,name,min,max,conn);
    ...
    public void createPartition(Groupe pere, String name, int n){
        ...
        Groupe copiePereRacinePartition = new GroupeP(pere,conn);
        ...
        for(int i = 0; i<n; i++){
            Groupe g = new GroupeP(copiePereRacinePartition,name+"_"+(i+1),min,max,conn); //modif : i+1 au lieu de i car TD_0 pas naturel
